...
- 计组刚开始会有点绕，我也是靠画图 + 自己搭 CPU 才慢慢顺起来的。从局部到整体，这样会好理解很多，如果难以理解不妨尝试下面两个实践练练手
- 🏗️ 可以尝试自己利用logisim构建一个属于自己的单周期甚至流水线的CPU进行实战
- 🔌 可以利用Verilog尝试搭建一个CPU，在另一个仓库[CPU](https://github.com/jlu005807/CPU),我已经尝试编写了两个五级流水线加上分支预测以及数据前推的CPU分别是MIPS32和LA32R，但是仅仅测试了其中一些指令。设计简陋仅供参考，存在大量问题还请见谅。
