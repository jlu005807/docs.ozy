# 🏗️ 数据结构（Data structure）

- 📘 本文件夹包含常见数据结构的简明说明和示例，适合快速回顾或入门学习。
- 🧩 对于每一个数据结构，章节里都有进行详细的介绍和具体的代码，对于每一种数据结构，首先是定义，然后用代码构建起来，最后则是利用一些对于数据结构的操作应用到实际问题的解决，也就是**程序 = 数据结构+算法**。
- 💻 更新了计算机23级数据结构的上机实验题目及代码，方便大家动手演练，熟悉各种数据结构的底层实现与应用。但是仅供参考，并且可能并不是每一届的题目都一样，并且我可不保证我的答案一定正确✔️
- 🔗 不过如果你想直接在本地运行对应数据结构的代码，可以查看我的github中对应的数据结构的仓库部署到本地运行。例如[Graph](https://github.com/jlu005807/Graph),[Tree](https://github.com/jlu005807/Tree)，其余的仓库详见[我的github主页](https://github.com/jlu005807)

章节列表与简短说明：

- `01_Introduction`：概述数据结构为何重要，以及常见分类。
- `02_Linear_List`：讲线性表（如数组、链表）的基本概念和常见操作。
- `03_Stack_and_Queue`：介绍栈和队列的用途与简单实现场景。
- `04_Array_and_Matrix`：说明数组与矩阵的表示与常见操作。
- `05_Character_String`：讲字符串的基本操作与常见问题处理方法。
- `06_Tree`：介绍树结构，常见的遍历与用途。
- `07_Graph`：说明图的基本概念、表示方法和常见算法场景。
- `08_Sort`：总结常见排序方法和适用场景。
- `09_Search`：讲查找技巧和常见的搜索算法。
- `data_structure`：对本章节合集的总览或总结主页用于复习。如果想保存到本地，可以直接保存这个文件即可

上机作业与练习：
- `计算机23级数据结构上机实验（第1-2周）`
- `计算机23级数据结构上机实验（第3-4周）`
- `计算机23级数据结构上机实验（第5-6周）`
- `计算机23级数据结构上级实验（第7周）`
- `计算机23级数据结构上级实验（第8周）`

