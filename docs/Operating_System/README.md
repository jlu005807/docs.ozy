...
- OS 确实有点枯燥并且很难去具象化实际的场景，但啃下来之后很多系统细节会豁然开朗。

子文件（主要内容）：

- `01_概论`：介绍操作系统的作用、组成与基本术语。
- `02_进程管理`：讲进程和线程的概念、创建与调度基础，以及进程间通信的常见方式。
- `03_中断和处理器调度`：解释中断机制和调度策略，说明操作系统如何响应外部事件并分配 CPU。
- `04_互斥同步与通讯`：介绍并发控制的方法（如锁、信号量）和进程/线程间通讯的常用技术。
- `05_死锁与饥饿`：说明死锁的形成条件、检测与避免策略，以及资源分配的公平性问题。
- `06_主存管理`：讲内存分配、管理策略与碎片问题。
- `07_虚拟存储器管理`：解释分页、换页和缓存策略，帮助理解内存虚拟化的工作原理。
- `08_文件与文件系统`：介绍文件抽象、目录结构与常见文件系统实现要点。
- `09_设备与IO管理`：说明设备驱动、IO 调度和常见外设管理方法。
- `review`：复习要点汇总。
